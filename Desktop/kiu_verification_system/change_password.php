...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (!empty($new_password) && strlen($new_password) < PASSWORD_MIN_LENGTH) {
            $validator->addError('new_password_len', 'New password must be at least ' . PASSWORD_MIN_LENGTH . ' characters.');
        }

        if ($new_password !== $confirm_password) {
            $validator->addError('password_match', 'New password and confirmation do not match.');
        }
        ...
        if (!$validator->hasErrors()) {
            ...
            $error = implode(' ', array_values($validator->getErrors()));
        }
    }
}
