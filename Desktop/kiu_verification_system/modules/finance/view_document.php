<?php
/**
 * Secure document viewer for Finance users.
 */
...
$storageRoots = array_filter([
    realpath(UPLOAD_DIR),
    defined('SECURE_UPLOAD_DIR') ? realpath(SECURE_UPLOAD_DIR) : false
]);

if (empty($storageRoots)) {
    http_response_code(500);
    echo 'Document storage is unavailable.';
    exit;
}

$storageRootsNormalized = array_map(function ($root) {
...
if ($relativePath !== '') {
    $candidatePaths[] = str_replace('\\', '/', SITE_ROOT) . '/' . $relativePath;
    foreach ($storageRootsNormalized as $root) {
        $candidatePaths[] = $root . '/' . $relativePath;
    }
}
...
foreach ($candidatePaths as $candidatePath) {
    ...
    foreach ($storageRootsNormalized as $root) {
        ...
        if (
            $normalizedCandidateLower === $rootLower ||
            strpos($normalizedCandidateLower, $rootLower . '/') === 0
        ) {
