    ...
    try {
        if ($action === 'create_user') {
            ...
            if ($fullName !== '' && table_exists($db, 'student_profiles')) {
                $activeIntakes = get_active_intakes($db);
                $defaultIntake = !empty($activeIntakes) ? $activeIntakes[0] : null;
                $defaultIntakeId = $defaultIntake ? (int)$defaultIntake['intake_id'] : null;
                $defaultIntakeYear = $defaultIntake ? (int)$defaultIntake['intake_year'] : (int)date('Y');
                $profileColumns = [
                ...
                $profileParams['intake_semester'] = 'semester_1';
                $profileParams['student_type'] = 'undergraduate';
                $profileParams['study_mode'] = 'full_time';

                if (column_exists($db, 'student_profiles', 'intake_id')) {
                    $profileColumns[] = 'intake_id';
                    $profileParams['intake_id'] = $defaultIntakeId;
                }
                if (column_exists($db, 'student_profiles', 'year_of_study')) {
                    $profileColumns[] = 'year_of_study';
                    $profileParams['year_of_study'] = 1;
                }
                ...
                $profileStmt = $db->prepare(
                    'INSERT INTO student_profiles (' . implode(', ', $profileColumns) . ') VALUES (' . implode(', ', $placeholders) . ')'
                );
