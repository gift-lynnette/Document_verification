...
$extracted_currency = in_array(($slip_amount['currency'] ?? ''), ['UGX', 'USD'], true)
    ? $slip_amount['currency']
    : $submission_currency;
$display_amount = $extracted_amount_raw;
if ($display_amount <= 0) {
    $display_amount = $submission['payment_amount'] ?? 0;
}
$display_currency = in_array(($submission['payment_currency'] ?? ''), ['UGX', 'USD'], true)
    ? ($slip_amount['source'] !== 'document_submission_ocr' ? $extracted_currency : $submission['payment_currency'])
    : $extracted_currency;
$display_amount_ugx = convert_amount_to_ugx($display_amount, $display_currency, $semester_exchange_rate);
$is_half_payment = abs($display_amount_ugx - $minimum_payment) < 0.01;
$remaining_balance_ugx = $is_half_payment
    ? max(0, $required_amount - $display_amount_ugx)
    : null;
$amount_paid_ugx = convert_amount_to_ugx($extracted_amount_raw, $extracted_currency, $semester_exchange_rate);
$eligibility_status = $display_amount_ugx + 0.01 >= $minimum_payment;
$eligibility_label = $eligibility_status
    ? 'Student eligible (>= 50% fee)'
    : 'Student not eligible (< 50% fee)';
...
$preview_outstanding = max(0, $preview_threshold - $display_amount_ugx);
$preview_decision = $display_amount_ugx >= $preview_threshold
    ? 'APPROVED'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_payment'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($fee_structure)) {
            $validator->addError('fee_structure', 'No active fee structure found for this programme and semester.');
        }

        if (!$slip_addressed_to_kiu || !$slip_has_bank_stamp || !$slip_has_valid_date || !$slip_legible) {
            $validator->addError('bank_slip_checks', 'Bank slip must be addressed to KIU, stamped, dated, and legible before processing.');
        }

        if ($finance_director_override && empty($finance_director_override_reason)) {
            $validator->addError('finance_director_override_reason', 'Finance Director override reason is required.');
        }
        
        if ($manual_override && empty($override_reason)) {
            $validator->addError('override_reason', 'Override reason is required');
        }

        if ($deferral_approved && empty($deferral_reason)) {
            $validator->addError('deferral_reason', 'Deferral reason is required');
        }
        ...
        if (
            ...
            $validator->addError('minimum_payment', 'Cannot approve: verified amount is below the required policy threshold.');
        }
        ...
        if ($decision === 'reject' && empty($rejection_reason)) {
            $validator->addError('rejection_reason', 'Rejection reason is required');
        }

        if ($is_pending && empty($pending_reason)) {
            $validator->addError('pending_reason', 'Pending reason is required');
        }
        ...
        $verification_summary = [
            ...
            'Intake' => trim(implode(' - ', array_filter([
...
<div class="container review-page-start">
    <div class="page-header">
        <h1><?php echo ui_icon('payment', 'title-icon'); ?>Payment Clearance Verification</h1>
    ...
    <div class="verification-container">
        ...
        <div class="info-card">
            ...
            <table class="info-table">
                ...
                <tr>
                    ...
                    <td><strong><?php echo htmlspecialchars($submission['registration_number'] ?? 'Pending'); ?></strong></td>
                </tr>
                <tr>
                    <th>Email:</th>
        ...
        <div class="info-card finance-status-card">
            <h2><?php echo ui_icon('check', 'title-icon'); ?>Admissions Office Status</h2>
            <div class="alert alert-success finance-inline-success">
        ...
        <div class="info-card">
            <h2><?php echo ui_icon('chart', 'title-icon'); ?>Fee Structure</h2>
            <table class="info-table">
                <tr>
        ...
        <!-- Payment Information -->
        <div class="info-card">
            <h2><?php echo ui_icon('payment', 'title-icon'); ?>Payment Information Submitted</h2>
            <table class="info-table">
                <tr>
                ...
                <tr>
                    ...
                    <td>UGX <?php echo number_format($required_amount, 2); ?></td>
                </tr>
                <?php
                    $remaining_display = max(0, $required_amount - $display_amount_ugx);
                ?>
                <tr>
                    <th>50% Fee Requirement:</th>
                    <td>UGX <?php echo number_format($minimum_payment, 2); ?></td>
                </tr>
                <tr>
                    <th>50% Calculation:</th>
                    <td>
                        UGX <?php echo number_format($required_amount, 2); ?>
                        x 50% = UGX <?php echo number_format($minimum_payment, 2); ?>
                    </td>
                </tr>
                <tr>
                    <th>Eligibility:</th>
                    <td><?php echo htmlspecialchars($eligibility_label); ?></td>
                </tr>
                <tr>
                    <th>Eligibility Check:</th>
                    <td>
                        UGX <?php echo number_format($display_amount_ugx, 2); ?>
                        <?php echo $eligibility_status ? '>=' : '<'; ?>
                        UGX <?php echo number_format($minimum_payment, 2); ?>
                    </td>
                </tr>
                <tr>
                    <th>Balance After Payment:</th>
                    <td>UGX <?php echo number_format($remaining_display, 2); ?></td>
                </tr>
                <tr>
                    <th>Calculation:</th>
                    <td>
                        UGX <?php echo number_format($required_amount, 2); ?>
                        - UGX <?php echo number_format($display_amount_ugx, 2); ?>
                        = UGX <?php echo number_format($remaining_display, 2); ?>
                    </td>
                </tr>
                <?php if ($submission_currency === 'USD'): ?>
                <tr>
                    <th>Intake FX Rate:</th>
            ...
            <div class="document-view">
                ...
                <a href="<?php echo BASE_URL; ?>modules/finance/view_document.php?id=<?php echo (int)$submission['submission_id']; ?>&doc=admission_letter" 
                         target="_blank" class="btn btn-primary doc-view-btn"><?php echo ui_icon('document', 'inline-icon'); ?>View Admission Letter</a>
                <?php endif; ?>
                ...
                <a href="<?php echo BASE_URL; ?>modules/finance/view_document.php?id=<?php echo (int)$submission['submission_id']; ?>&doc=bank_slip" 
                         target="_blank" class="btn btn-primary doc-view-btn"><?php echo ui_icon('document', 'inline-icon'); ?>View Bank Slip</a>
                ...
                <a href="<?php echo BASE_URL; ?>modules/finance/view_document.php?id=<?php echo (int)$submission['submission_id']; ?>&doc=bursary_award_letter" 
                         target="_blank" class="btn btn-primary doc-view-btn"><?php echo ui_icon('document', 'inline-icon'); ?>View Award Letter</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="info-card">
            <h2><?php echo ui_icon('check', 'title-icon'); ?>Payment Clearance Decision</h2>
            <div class="payment-analysis">
                ...
                <p><strong>Intake:</strong> <?php echo htmlspecialchars(trim(implode(' - ', array_filter([
                    (string)($submission['intake_name'] ?? ''),
                    (string)($submission['intake_year'] ?? '')
                ])))); ?></p>
                ...
                <p><strong>Amount Paid (Bank Slip):</strong> <?php echo htmlspecialchars($display_currency); ?> <?php echo number_format((float)$display_amount, 2); ?></p>
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="payment_amount_verified" step="0.01" 
                        value="<?php echo htmlspecialchars((string)$display_amount); ?>"
                ...
                <div class="payment-analysis">
                    ...
                    <p>Status: 
                        <span class="badge badge-<?php echo $meets_minimum ? 'success' : 'warning'; ?>">
                            <?php echo $meets_minimum ? ui_icon('check', 'inline-icon') . 'Meets Minimum' : ui_icon('warning', 'inline-icon') . 'Below Minimum'; ?>
                        </span>
                    </p>
                </div>
                <?php endif; ?>
                ...
                <div class="form-actions">
                    <button type="submit" name="verify_payment" class="btn btn-primary submit-decision-btn">
                        <?php echo ui_icon('payment', 'inline-icon'); ?>Submit Clearance Decision
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="alert alert-info">
            <strong><?php echo ui_icon('info', 'inline-icon'); ?>Already Processed</strong><br>
