...
$reg = strtoupper(trim((string)($_GET['reg'] ?? '')));
$card = strtoupper(trim((string)($_GET['card'] ?? '')));

if ($token !== '' && !preg_match('/^[A-Za-z0-9_-]{20,120}$/', $token)) {
    $token = '';
}
if ($reg !== '' && !preg_match('/^[A-Z0-9\/\-]{4,50}$/', $reg)) {
    $reg = '';
}
if ($card !== '' && !preg_match('/^GC[0-9]{4,10}$/', $card)) {
    $card = '';
}
...
if ($token !== '' || $reg !== '' || $card !== '') {
    try {
        if ($token !== '' && table_exists($db, 'green_cards') && column_exists($db, 'green_cards', 'verification_token_hash')) {
            $stmt = $db->prepare("
                SELECT gc.card_id, gc.card_number, gc.registration_number,
                       ...
                       gc.is_active,
                       u.user_id, u.admission_number
                FROM green_cards gc
                LEFT JOIN users u ON gc.user_id = u.user_id
                WHERE gc.verification_token_hash = :token_hash
                LIMIT 1
            ");
            $stmt->execute(['token_hash' => gc_hash_verification_token($token)]);
            $record = $stmt->fetch(PDO::FETCH_ASSOC);
        }

        if (!$record && $token !== '' && table_exists($db, 'green_cards')) {
            $stmt = $db->prepare("
                SELECT gc.card_id, gc.card_number, gc.registration_number,
                       gc.full_name, gc.program, gc.faculty, gc.student_photo_path,
                       gc.issue_date, gc.expiry_date, gc.academic_year, gc.semester,
                ...
                FROM green_cards gc
                LEFT JOIN users u ON gc.user_id = u.user_id
                WHERE gc.qr_code_data LIKE :token_match
                LIMIT 1
            ");
            $stmt->execute(['token_match' => '%' . $token . '%']);
            $record = $stmt->fetch(PDO::FETCH_ASSOC);
        } elseif ($card !== '') {
            $stmt = $db->prepare("
                SELECT gc.card_id, gc.card_number, gc.registration_number,
            ...
            $stmt = $db->prepare("
                ...
                FROM green_cards gc
                LEFT JOIN users u ON gc.user_id = u.user_id
                WHERE gc.registration_number = :reg
                LIMIT 1
            ");
...
<main class="container">
    ...
    <form method="GET" class="card form-section <?php echo ($token === '' && $reg === '' && $card === '') ? 'visible' : ''; ?>" style="padding:20px;margin:20px 0;">
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="reg" value="<?php echo htmlspecialchars($reg); ?>" placeholder="e.g. 2026-08-1001">
        </div>
        <button class="btn btn-primary" type="submit">Verify</button>
    </form>

    <?php if ($token !== '' || $reg !== '' || $card !== ''): ?>
        ...
        <?php if ($status === 'not_found'): ?>
            <div class="verification-card" style="border-color: #fee2e2; background: #fef2f2;">
                <p style="color: #991b1b; margin: 0; font-size: 14px;">
                    ...
                    <small>No matching active green card record was found.</small>
                </p>
            </div>
        <?php endif; ?>
    <?php endif; ?>
